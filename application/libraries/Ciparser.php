...
class Ciparser 
{
	...
	private function _new_parse($template, $data, $key, $embed_template)
	{
		...
		$module_user = $keys[1];
		$module_name = $keys[2];
		$this->_new_parse_single($data, $module_place,$module_user, $module_name, $embed_template, $template);
	}
	
	// now parsing a tag
	private function _new_parse_single($data, $module_place,$module_user, $module_name, $embed_template, $string)
	{
		// find ci:doc tag on view
		$regex = '#<ci:doc type( =|=|= )("|\')('.$module_place.')("|\')( \/|\/)>#';
		
		// replace ci:doc tag with view file of selected module
		$replace = realpath(APPPATH . '/' . $module_place . '/' .$module_user. '/'. $module_name .'/views/' . $embed_template . '.php');
